...
const excelDateToISO = (excelDate: number | string): string | undefined => {
  ...
  if (typeof excelDate === 'string') {
    ...
    // Filtrar valores no-fecha explícitamente (spec v2)
    if (trimmed === '*' || trimmed === '＊') return undefined;
    if (/^\d+日間$/.test(trimmed)) return undefined;          // "7日間" - marcador consecutivo
    if (trimmed.includes('消滅')) return undefined;            // "2022/8/1消滅" - prescripción

    // Limpiar sufijo 半休 (half-day marker)
    let cleanValue = trimmed.replace(/半休?/g, '').trim();

    // Limpiar nota de pago 支給: "4/20(5/15支給)" → "4/20"
    cleanValue = cleanValue.replace(/[（(][^）)]*支給[^）)]*[）)]/g, '').trim();

    // Si es un número como string (e.g., "44881" de export previo)
    ...
    // Intentar parsear formato YYYY/MM/DD o YYYY-MM-DD
    const matchFull = cleanValue.match(/(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})/);
    if (matchFull) {
      const y = matchFull[1];
    ...
    // Formato corto M/D sin año: necesita contexto → no parsear
    // String no reconocido como fecha válida → retornar undefined
    return undefined;
  }
...
// Extraer fechas de yukyu (detecta 半休/半 para half-day, filtra *, X日間, 消滅)
const extractYukyuDates = (row: any): string[] => {
  ...
  for (let i = 1; i <= 40; i++) {
    ...
    // Filtros explícitos para no-fechas (spec v2)
    if (strVal === '*' || strVal === '＊') continue;
    if (/^\d+日間$/.test(strVal)) continue;
    if (strVal.includes('消滅')) continue;

    // Detectar medio día: 半休 o 半 (sin ser parte de otra palabra)
    const isHalf = typeof value === 'string' && /半/.test(strVal);
    const dateStr = excelDateToISO(value);
    if (dateStr) dates.push(isHalf ? `${dateStr}:half` : dateStr);
  }
  return dates;
};
...
const buildPeriodHistory = (
  ...
  rows.forEach((row, index) => {
    ...
    // Extraer fechas de uso ANTES de calcular used (spec v2: no confiar en COUNTA)
    const rowYukyuDates = extractYukyuDates(row);

    // Recalcular used desde slots reales (COUNTA sobrecontea *, X日間, 消滅)
    const usedFromSlots = rowYukyuDates.reduce((sum, d) =>
      sum + (d.endsWith(':half') ? 0.5 : 1.0), 0);
    const excelUsed = Number(findValue(row, ['消化日数', '消化合計', '使用日数'])) || 0;
    // Preferir cálculo propio; fallback a Excel solo si no hay slots
    const used = usedFromSlots > 0 ? usedFromSlots : excelUsed;

    const balance = granted - used - expired + (carryOver || 0);

    // Calcular fechas
    const yukyuStartDate = yukyuStartDateRaw ? excelDateToISO(yukyuStartDateRaw) : undefined;

    // Calcular grantDate evitando overflow de setMonth (ej: ene 31 + 1 mes = mar 3)
    const grantDate = new Date(entry.getFullYear(), entry.getMonth() + elapsedMonths, entry.getDate());
    // Si el día se desbordó al mes siguiente, retroceder al último día del mes correcto
    const expectedMonth = (entry.getMonth() + elapsedMonths) % 12;
    if (grantDate.getMonth() !== expectedMonth) {
      grantDate.setDate(0); // Último día del mes anterior
    }

    const expiryDate = new Date(grantDate.getFullYear() + 2, grantDate.getMonth(), grantDate.getDate());
    if (expiryDate.getMonth() !== grantDate.getMonth()) {
    ...
    // rowYukyuDates ya fue extraído arriba (antes del cálculo de used)

    // DEDUPLICACIÓN: Si ya existe este período, mergear fechas y tomar max de numéricos
    if (periodMap.has(elapsedMonths)) {
      ...
      // Tomar el mayor valor numérico (la fila con datos más completos)
      existing.granted = Math.max(existing.granted, granted);
      existing.used = Math.max(existing.used, used);
      // ⭐ FIX: Recalcular balance como granted - used (no Math.max)
      existing.balance = existing.granted - existing.used;
      existing.expired = Math.max(existing.expired, expired);
...
const processYukyu = (
  ...
  YUKYU_SHEETS.forEach(({ name: sheetName, category }) => {
    ...
    const sheet = workbook.Sheets[sheetName];
    // Spec v2: Headers en fila 5 para 作業者データ　有給, fila 4 para 請負
    const headerRow = sheetName === '請負' ? 3 : 4; // 0-indexed: fila 4→3, fila 5→4
    const jsonData = XLSX.utils.sheet_to_json<any>(sheet, { range: headerRow });
    if (jsonData.length === 0) return;
  ...
  employeeYukyuMap.forEach(({ allRows, allYukyuDates, category, status }, id) => {
    ...
    // ⭐ LEGACY: Tomar valores del período más reciente (no sumar todos)
    const lastRow = allRows[allRows.length - 1];
    const carryOver = Number(findValue(lastRow, ['繰越', '繰越日数'])) || 0;
    const totalAvailable = Number(findValue(lastRow, ['保有数', '保有日数'])) || 0;
    const remainingAfterExpiry = Number(findValue(lastRow, ['時効後残', '時効後残日数'])) || 0;
    ...
    if (existingIdx >= 0) {
      ...
      const excelData: Partial<Employee> = {
        ...
        // ⭐ LEGACY: Campos para backward compatibility (ambos current para coherencia)
      ...
      existingEmployees.push({
        ...
        // ⭐ Valores ACTUALES (solo períodos vigentes) - NO usar || undefined (0 es válido)
        currentGrantedTotal,
        currentUsedTotal,
        currentBalance,
        currentExpiredCount,

        // ⭐ Valores HISTÓRICOS (todos los períodos)
        historicalGrantedTotal,
        historicalUsedTotal,
        historicalBalance,
        historicalExpiredCount,
...
const Dropzone: React.FC<DropzoneProps> = ({ type, title, subtitle, icon, color, syncStatus, onProcess, loading, progress, includeResigned, isDark }) => {
  ...
  return (
    <div
      ...
      className={`relative border-2 border-dashed p-4 md:p-6 lg:p-8 text-center transition-all duration-500 rounded-lg ${
...
const ExcelSync: React.FC<ExcelSyncProps> = ({ onSyncComplete }) => {
  ...
  const processDaichoFile = (file: File) => {
    ...
    // Upload to backend in parallel (fire-and-forget for now)
    api.syncExcel(file, 'shaintaicho').catch(err => {
  ...
  const processYukyuFile = (file: File) => {
    ...
    // Upload to backend in parallel (fire-and-forget for now)
    api.syncExcel(file, 'yukyu').catch(err => {
      console.warn('Backend sync failed (non-blocking):', err);
    });
  ...
  return (
    <div className="p-4 md:p-8 lg:p-12 max-w-[1200px] mx-auto space-y-6 md:space-y-8 lg:space-y-12 animate-fadeIn relative pb-20 lg:pb-32">
      <div className={`absolute top-0 right-0 text-[18vw] font-black select-none pointer-events-none italic tracking-tighter hidden md:block ${isDark ? 'text-white/[0.01]' : 'text-slate-900/[0.02]'}`}>同期</div>
      ...
      <header className={`flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-8 relative z-10 border-b pb-6 md:pb-8 lg:pb-12 ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
        <div className="space-y-2 md:space-y-4">
          <div className="flex items-center gap-3 md:gap-6">
            <div className="h-8 md:h-14 w-1.5 md:w-2 bg-blue-500 shadow-[0_0_20px_#00e5ff] animate-pulse"></div>
            <h2 className={`text-3xl md:text-5xl lg:text-7xl font-black italic tracking-tighter ${isDark ? 'aggressive-text' : 'text-slate-800'}`}>データ同期</h2>
          </div>
          <div className={`flex items-center gap-2 md:gap-4 font-black tracking-[0.2em] md:tracking-[0.4em] ml-4 md:ml-8 text-[10px] md:text-sm ${isDark ? 'text-white/30' : 'text-slate-500'}`}>
            <span>2ファイル対応</span>
            <span className="text-blue-500">●</span>
            <span className="hidden sm:inline">社員台帳 + 有給休暇管理</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 relative z-10">
        <div className={`p-6 md:p-10 border space-y-2 md:space-y-4 group hover:border-green-500/50 transition-colors ${isDark ? 'bg-[#0a0a0a] border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
          <div className="text-green-500 font-black text-lg md:text-xl italic tracking-tighter">01. 社員台帳</div>
          <p className={`text-[11px] md:text-xs leading-relaxed font-bold ${isDark ? 'text-white/40' : 'text-slate-500'}`}>
            社員の基本情報（社員№、氏名、派遣先、ステータス）を取り込みます。派遣・請負・スタッフの3カテゴリに対応。
          </p>
        </div>
        <div className={`p-6 md:p-10 border space-y-2 md:space-y-4 group hover:border-blue-500/50 transition-colors ${isDark ? 'bg-[#0a0a0a] border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
          <div className="text-blue-500 font-black text-lg md:text-xl italic tracking-tighter">02. 有給休暇管理</div>
          <p className={`text-[11px] md:text-xs leading-relaxed font-bold ${isDark ? 'text-white/40' : 'text-slate-500'}`}>
            有給発生日、付与数、消化日数、期末残高、時効数、取得日一覧を完全に取り込みます。
          </p>
        </div>
        <div className={`p-6 md:p-10 border space-y-2 md:space-y-4 group transition-colors ${isDark ? 'bg-[#0a0a0a] border-white/5 hover:border-white/50' : 'bg-white border-slate-200 shadow-sm hover:border-slate-400'}`}>
          <div className={`font-black text-lg md:text-xl italic tracking-tighter ${isDark ? 'text-white' : 'text-slate-800'}`}>03. データ統合</div>
          <p className={`text-[11px] md:text-xs leading-relaxed font-bold ${isDark ? 'text-white/40' : 'text-slate-500'}`}>
