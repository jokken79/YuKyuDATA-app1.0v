...
const ApplicationManagement: React.FC<ApplicationManagementProps> = ({ data, onUpdate }) => {
  ...
  const handleApprove = async (recordId: string) => {
    const result = await api.approveRecord(recordId);
  ...
  const handleReject = async (recordId: string) => {
    ...
    const success = await api.rejectRecord(recordId, reason || undefined);
    if (success) {
      onUpdate();
      setSelectedIds(prev => {
        const next = new Set(prev);
  ...
  const confirmBulkApprove = async () => {
    ...
    const results = await api.approveMultiple(pendingSelected);
  ...
  const handleBulkReject = async () => {
    ...
    if (reason !== null) {
      const count = await api.rejectMultiple(pendingSelected, reason || undefined);
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredRecords.map(r => {
      ...
      return [
        ...
        r.note || ''
      ].map(val => `"${String(sanitizeValue(val)).replace(/"/g, '""')}"`).join(',');
    });
