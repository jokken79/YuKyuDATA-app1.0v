    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'client':
          ...
          break;
        case 'grantedTotal':
          comparison = (a.currentGrantedTotal ?? a.grantedTotal ?? 0) - (b.currentGrantedTotal ?? b.grantedTotal ?? 0);
          break;
        case 'usedTotal':
          comparison = (a.currentUsedTotal ?? a.usedTotal ?? 0) - (b.currentUsedTotal ?? b.usedTotal ?? 0);
          break;
        case 'balance':
          comparison = (a.currentBalance ?? a.balance ?? 0) - (b.currentBalance ?? b.balance ?? 0);
          break;
        case 'status':
        ...
        <FocusTrap>
          ...
          >
            ...
            <div>
              ...
              {selectedEmployee.periodHistory && selectedEmployee.periodHistory.length > 0 ? (
              ...
              ) : selectedEmployee.yukyuDates && selectedEmployee.yukyuDates.length > 0 ? (
                // Fallback: mostrar yukyuDates agrupadas por año cuando no hay periodHistory
                <div className="space-y-4">
                  {(() => {
                    const datesByYear: Record<string, string[]> = {};
                    selectedEmployee.yukyuDates!.forEach(dateStr => {
                    ...
                    return Object.entries(datesByYear)
                      .sort(([a], [b]) => b.localeCompare(a))
                      .map(([year, dates]) => (
                        <div key={year} className="space-y-2">
                          <div className={`flex items-center justify-between py-2 px-3 rounded-lg bg-blue-500/10 border border-blue-500/20`}>
                            <span className="text-sm font-black text-blue-400">{year}年</span>
                            <span className={`text-xs ${isDark ? 'text-white/80' : 'text-slate-400'}`}>({dates.reduce((s, d) => s + (d.endsWith(':half') ? 0.5 : 1), 0)}日消化)</span>
                          </div>
                          <div className="grid grid-cols-3 md:grid-cols-4 gap-1 pl-2">
                            {dates.sort().map((dateStr, i) => {
                              const isHalf = dateStr.endsWith(':half');
                              const dateOnly = isHalf ? dateStr.replace(':half', '') : dateStr;
                              return (
                                <div
                                  key={`${dateStr}-${i}`}
                                  className={`text-[11px] font-mono py-1.5 px-2 rounded text-center ${isDark ? 'bg-white/10 text-white/80' : 'bg-slate-50 text-slate-600'}`}
                                >
                                  {new Date(dateOnly).toLocaleDateString('ja-JP', { month: 'short', day: 'numeric' })}
                                  {isHalf && <span className="ml-0.5 text-[9px] opacity-60">(半)</span>}
                                </div>
                  ...
                  <div className={`mt-4 p-3 rounded-xl ${isDark ? 'bg-amber-500/5 border border-amber-500/10' : 'bg-amber-50 border border-amber-100'}`}>
                    <p className={`text-[10px] leading-relaxed ${isDark ? 'text-white/60' : 'text-slate-600'}`}>
                      <span className="text-amber-500 font-bold">注意:</span> 年別表示（期間別データなし）。有給休暇管理.xlsmを同期すると期間別に表示されます。
                    </p>
                  </div>
                </div>
              ) : (
                <div className="py-8 text-center space-y-2">
                  ...
                  <p className={`text-sm ${isDark ? 'text-white/70' : 'text-slate-400'}`}>有給取得データなし</p>
