...
const categorizeDatesByEntryAnniversary = (dates: string[], entryDate: string | undefined): YearGroup[] => {
  ...
  dates.forEach(dateStr => {
    // Limpiar sufijo :half para parsear la fecha correctamente
  ...
  sortedPeriods.forEach(periodIndex => {
    const periodDates = yearGroups.get(periodIndex)!.sort((a, b) => {
      const dateA = a.replace(/:half$/, '');
      const dateB = b.replace(/:half$/, '');
...
const LeaveRequest: React.FC<LeaveRequestProps> = ({ data, onSuccess }) => {
  ...
  const allLeaveHistory = useMemo(() => {
    ...
    // Fechas de records de la app (incluir :half para consistencia)
    const appDates = data.records
      .filter(r => r.employeeId === selectedEmployee.id && r.type === 'paid')
      .map(r => r.duration === 'half' ? `${r.date}:half` : r.date);
  ...
  const calculatedTotals = useMemo(() => {
    ...
    // Helper: contar valor real de días (half=0.5, full=1.0)
    const countDaysValue = (dates: string[]) =>
      dates.reduce((sum, d) => sum + (d.endsWith(':half') ? 0.5 : 1), 0);

    // Sumar todos los días consumidos (half=0.5)
    const totalUsed = historyByYear
      .reduce((sum, period) => sum + countDaysValue(period.dates), 0);
    ...
    const expiredCount = historyByYear
      .filter(period => period.isExpired)
      .reduce((sum, period) => {
        const usedInPeriod = countDaysValue(period.dates);
        const unusedInPeriod = period.daysGranted - usedInPeriod;
  ...
  const analysis = useMemo(() => {
    ...
    // Contar desde allLeaveHistory (limpiar :half antes de parsear fecha)
    const validDates = allLeaveHistory.filter(d => new Date(d.replace(/:half$/, '')) >= twoYearsAgo);
    const recentDates = allLeaveHistory.filter(d => new Date(d.replace(/:half$/, '')) >= last30Days);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const employee = data.employees.find(e => e.id === formData.employeeId);
      ...
      setFormData(prev => ({ ...prev, note: '', date: new Date().toISOString().split('T')[0], duration: 'full' }));
      onSuccess();
            ...
            <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  aria-describedby={selectedEmployee && allLeaveHistory.some(d => d.replace(/:half$/, '') === formData.date) ? "date-warning" : undefined}
                  aria-invalid={selectedEmployee && allLeaveHistory.some(d => d.replace(/:half$/, '') === formData.date) ? "true" : "false"}
                  className={`w-full rounded-xl px-4 py-3 focus:border-indigo-500 transition-all outline-none ${isDark ? 'bg-white/10 border border-white/20 text-white' : 'bg-white border border-slate-200 text-slate-800'}`}
                  required
                />
                {/* Warning if date already taken (check both full and half variants) */}
                {selectedEmployee && allLeaveHistory.some(d => d.replace(/:half$/, '') === formData.date) && (
              ...
              <div className={`p-4 rounded-2xl ${isDark ? 'bg-white/10 border border-white/20' : 'bg-slate-50 border border-slate-100'}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[0, 1].map(monthOffset => {
                    ...
                    // Get dates taken in this month (strip :half before parsing)
                    const takenInMonth = allLeaveHistory.filter(d => {
                      const cleanD = d.replace(/:half$/, '');
                      const date = new Date(cleanD);
                      return !isNaN(date.getTime()) && date.getFullYear() === year && date.getMonth() === month;
                    }).map(d => {
                      const cleanD = d.replace(/:half$/, '');
                      return new Date(cleanD).getDate();
                    });
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {selectedEmployee && allLeaveHistory.length > 0 && (
                <span className="text-xs bg-indigo-500/20 text-indigo-500 px-2 py-1 rounded-full font-bold">
                  計 {allLeaveHistory.reduce((sum, d) => sum + (d.endsWith(':half') ? 0.5 : 1), 0)} 日
                </span>
              )}
            </div>
                    ...
                    <div key={yearGroup.period} className="space-y-2">
                      ...
                      <div className={`flex items-center justify-between py-2 px-3 rounded-lg ${
                        ...
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs ${isDark ? 'text-white/80' : 'text-slate-400'}`}>({yearGroup.dates.reduce((s, d) => s + (d.endsWith(':half') ? 0.5 : 1), 0)}日消化)</span>
                          </div>
                      ...
                      <div className="grid grid-cols-3 gap-1 pl-2">
                        {yearGroup.dates.slice(0, 12).map((date, i) => {
                          const isHalf = date.endsWith(':half');
                          const dateOnly = isHalf ? date.replace(':half', '') : date;
                          return (
                          <div
                            key={`${date}-${i}`}
