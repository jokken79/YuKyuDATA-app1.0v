...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed lg:relative w-[75vw] max-w-[320px] lg:w-80 h-full cyber-glass flex flex-col z-50 border-r transition-transform duration-300 ease-in-out ${
      ...
      <div className="p-4 md:p-6 lg:p-10">
        <div className={`relative group overflow-hidden rounded-3xl p-4 md:p-6 lg:p-8 border glitch-hover transition-all duration-300 ${
          isDark
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="w-28 h-20 md:w-32 md:h-24 lg:w-40 lg:h-32 flex items-center justify-center">
              <img
                src="/uns-logo.png"
                alt="UNS - ユニバーサル企画"
              ...
              <div className="hidden flex items-center justify-center gap-0">
                <span className={`text-3xl md:text-4xl lg:text-5xl font-black italic tracking-tighter leading-none ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>U</span>
                <span className={`text-3xl md:text-4xl lg:text-5xl font-black italic tracking-tighter leading-none ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>N</span>
                <span className={`text-3xl md:text-4xl lg:text-5xl font-black italic tracking-tighter leading-none ${isDark ? 'text-red-500' : 'text-red-600'}`}>S</span>
              </div>
            </div>

            <div className="mt-2 md:mt-3 lg:mt-4 flex flex-col items-center">
              <div className={`text-[8px] md:text-[9px] lg:text-[10px] font-black tracking-[0.3em] md:tracking-[0.5em] mb-1 ${isDark ? 'text-white/80' : 'text-slate-500'}`}>ユニバーサル企画</div>
              <div className="h-0.5 w-full bg-gradient-to-r from-blue-500 via-white to-red-500"></div>
              <div className={`text-sm md:text-base lg:text-lg font-black italic tracking-tighter mt-1 md:mt-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>有給PRO V1.0</div>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        {tabs.map((tab) => (
          <button
            ...
            className={`w-full group flex items-center gap-4 lg:gap-6 px-4 lg:px-6 py-3 lg:py-4 rounded-2xl transition-all duration-300 relative ${
              activeTab === tab.id
