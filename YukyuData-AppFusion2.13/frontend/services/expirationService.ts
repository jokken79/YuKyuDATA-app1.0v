...
export function recalculateExpiration(employee: Employee): Employee {
  ...
  let currentUsedTotal = currentPeriods.reduce((sum, p) => sum + p.used, 0);

  // ⭐ FIX: Incluir aprobaciones locales no contabilizadas en period.used
  const localApproved = employee.localModifications?.approvedDates || [];
  if (localApproved.length > 0) {
    const allPeriodDatesSet = new Set(
      updatedPeriodHistory.flatMap(p => (p.yukyuDates || []).map(d => d.replace(/:half$/, '')))
    );
    let extraLocalUsed = 0;
    localApproved.forEach(d => {
      const isHalf = d.endsWith(':half');
      const clean = d.replace(/:half$/, '').replace(/\//g, '-');
      if (!allPeriodDatesSet.has(clean)) {
        extraLocalUsed += isHalf ? 0.5 : 1;
      }
    });
    if (extraLocalUsed > 0) {
      currentUsedTotal += extraLocalUsed;
      console.log(`📝 ${employee.name}: +${extraLocalUsed}日 aprobaciones locales no contabilizadas en períodos`);
    }
  }

  const rawCurrentBalance = currentGrantedTotal - currentUsedTotal;

  // ⭐ NUEVO: Aplicar límite legal de 40 días (労働基準法第115条)
  // Asegurar que el balance nunca sea negativo (protección contra datos corruptos)
  const LEGAL_MAX_BALANCE = 40;
  const currentBalance = Math.max(0, Math.min(rawCurrentBalance, LEGAL_MAX_BALANCE));
  ...
  // Incluir las mismas aprobaciones locales extra en históricos
  const baseHistoricalUsed = updatedPeriodHistory.reduce((sum, p) => sum + p.used, 0);
  const extraLocal = currentUsedTotal - currentPeriods.reduce((sum, p) => sum + p.used, 0);
  const historicalUsedTotal = baseHistoricalUsed + extraLocal;
  const historicalBalance = historicalGrantedTotal - historicalUsedTotal;
  ...
  return {
    ...
    // ⭐ LEGACY: Campos legacy consistentes (ambos current* para que balance = granted - used)
  ...
  YUKYU_GRANT_TABLE.forEach((grant, index) => {
    ...
    if (monthsFromEntry >= grant.elapsedMonths && !existingElapsedMonths.includes(grant.elapsedMonths)) {
      ...
      periodsToGenerate.push({
        ...
        source: 'generated', // Auto-generado por expirationService
