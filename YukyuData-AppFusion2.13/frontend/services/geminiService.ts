...
export const analyzeLeaveData = async (data: AppData): Promise<AIInsight[]> => {
  ...
  const riskEmployees = data.employees.filter(e => {
    const granted = e.currentGrantedTotal ?? e.grantedTotal;
    const used = e.currentUsedTotal ?? e.usedTotal;
    return e.status === '在職中' && granted >= 10 && used < 5;
  });
  ...
  const prompt = `
    ...
    【リスク対象者詳細】
    ${riskEmployees.slice(0, 10).map(e => {
      const used = e.currentUsedTotal ?? e.usedTotal;
      return `${e.name}: 消化済み${used}日 (残り${5 - used}日必要)`;
    }).join('\n')}
