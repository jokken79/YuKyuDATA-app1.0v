  ...
  data.records.forEach(record => {
    ...
    if (record.type === 'paid' && record.status === 'approved') {
      ...
      if (emp && emp.yukyuDates) {
        // Formato: preservar duración half-day
        const dateEntry = record.duration === 'half' ? `${record.date}:half` : record.date;

        // Verificar si la fecha ya está en yukyuDates (ambas variantes)
