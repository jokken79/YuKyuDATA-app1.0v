...
export const db = {
  ...
  loadData: (): AppData => {
    try {
      ...
      // Guardar datos con expiraciones recalculadas si hubo cambios
      const hasExpirationChanges = updatedEmployees.some((emp, i) => {
        const orig = migrationResult.data.employees[i];
        if (!orig) return true;
        return emp.currentBalance !== orig.currentBalance ||
               emp.currentExpiredCount !== orig.currentExpiredCount ||
               emp.isExpired !== (orig as any).isExpired;
      });
      if (hasExpirationChanges) {
  ...
  approveRecord: (recordId: string, approvedBy?: string): { success: boolean; error?: string; code?: string } => {
    ...
    if (record.type === 'paid' && emp) {
      ...
      // ⭐ NUEVO: Marcar como modificación local (incluir :half para consistencia con yukyuDates)
      if (!emp.localModifications) {
        emp.localModifications = { approvedDates: [], manualAdjustments: 0 };
      }
      if (!emp.localModifications.approvedDates.includes(dateEntry)) {
        emp.localModifications.approvedDates.push(dateEntry);
      }
