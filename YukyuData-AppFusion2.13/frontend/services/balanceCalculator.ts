 ...
 *
 * ⭐ REGLA LIFO (新しい方から消化):
 * Los días consumidos se descuentan del período MÁS NUEVO primero.
 * Solo cuando el período nuevo se agota, se usa el anterior.
 * Los períodos antiguos NUNCA se borran (historial completo).
 */
export function getEmployeePeriods(employee: Employee): PeriodInfo[] {
  ...
  // Contar total de días consumidos desde yukyuDates[]
  const totalUsed = countDaysValue(yukyuDates);
  ...
  for (let i = 0; i <= currentPeriodIndex; i++) {
    ...
    periods.push({
      ...
      expiryDate,
      daysGranted,
      daysUsed: 0, // Se asigna abajo con distribución LIFO
      isExpired,
      isCurrentPeriod
    });
  }

  // ⭐ LIFO: Distribuir días consumidos desde el período más nuevo al más antiguo
  let remainingToDistribute = totalUsed;

  // 1. Primero: períodos vigentes (no expirados), del más nuevo al más antiguo
  const nonExpiredNewestFirst = periods
    .filter(p => !p.isExpired)
    .sort((a, b) => b.periodIndex - a.periodIndex);

  for (const period of nonExpiredNewestFirst) {
    if (remainingToDistribute <= 0) break;
    const canUse = Math.min(remainingToDistribute, period.daysGranted);
    period.daysUsed = canUse;
    remainingToDistribute -= canUse;
  }

  // 2. Si sobran días, asignar a períodos expirados (del más nuevo al más antiguo)
  if (remainingToDistribute > 0) {
    const expiredNewestFirst = periods
      .filter(p => p.isExpired)
      .sort((a, b) => b.periodIndex - a.periodIndex);

    for (const period of expiredNewestFirst) {
      if (remainingToDistribute <= 0) break;
      const canUse = Math.min(remainingToDistribute, period.daysGranted);
      period.daysUsed = canUse;
      remainingToDistribute -= canUse;
...
export function isLegalRisk(employee: Employee): boolean {
  // Usar current* que solo cuentan períodos vigentes (no expirados)
  const granted = employee.currentGrantedTotal ?? employee.grantedTotal ?? 0;
  const used = employee.currentUsedTotal ?? employee.usedTotal ?? 0;
