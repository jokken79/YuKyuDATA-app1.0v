...
export function validateEmployeeData(employee: Employee): IntegrityCheck {
  ...
  // 9. yukyuDates (valor en días) vs currentUsedTotal
  if (
    employee.yukyuDates &&
    employee.currentUsedTotal !== undefined
  ) {
    // Calcular valor real: full=1, half=0.5
    const yukyuDaysValue = employee.yukyuDates.reduce((sum, d) =>
      sum + (d.endsWith(':half') ? 0.5 : 1), 0);
    if (Math.abs(yukyuDaysValue - employee.currentUsedTotal) > 3) {
      issues.push({
        code: 'YUKYU_DATES_MISMATCH',
        message: `yukyuDates(${yukyuDaysValue}日) ≠ currentUsedTotal(${employee.currentUsedTotal})`,
        severity: 'warning',
        field: 'yukyuDates',
        expected: employee.currentUsedTotal,
        actual: yukyuDaysValue
      });
    }
  }
