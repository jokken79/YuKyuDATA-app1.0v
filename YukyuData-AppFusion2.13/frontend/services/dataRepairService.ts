...
export function repairEmployeeData(employee: Employee): RepairResult {
  ...
  // Calcular used total de períodos vigentes + aprobaciones locales
  let calculatedCurrentUsed = currentPeriods.reduce((sum, p) => sum + p.used, 0);

  // ⭐ FIX: Incluir aprobaciones locales no contabilizadas (consistente con expirationService)
  const localApproved = repairedEmployee.localModifications?.approvedDates || [];
  if (localApproved.length > 0) {
    const allPeriodDatesSet = new Set(
      (repairedEmployee.periodHistory || []).flatMap(p => (p.yukyuDates || []).map(d => d.replace(/:half$/, '')))
    );
    localApproved.forEach(d => {
      const isHalf = d.endsWith(':half');
      const clean = d.replace(/:half$/, '').replace(/\//g, '-');
      if (!allPeriodDatesSet.has(clean)) {
        calculatedCurrentUsed += isHalf ? 0.5 : 1;
      }
    });
  }
  ...
  // Calcular balance (raw, sin límite legal aún) — consistente con expirationService
  const rawCurrentBalance = calculatedCurrentGranted - calculatedCurrentUsed;

  // Aplicar límite legal de 40 días (nunca negativo)
  const LEGAL_MAX_BALANCE = 40;
  const calculatedCurrentBalance = Math.max(0, Math.min(rawCurrentBalance, LEGAL_MAX_BALANCE));
  ...
  if (repairedEmployee.usedTotal !== calculatedCurrentUsed) {
    actions.push({
      field: 'usedTotal (legacy)',
      oldValue: repairedEmployee.usedTotal,
      newValue: calculatedCurrentUsed,
      reason: 'Sincronizar con currentUsedTotal (consistente con grantedTotal=current)'
    });
    repairedEmployee.usedTotal = calculatedCurrentUsed;
  }
