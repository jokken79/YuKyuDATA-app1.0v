...
export function mergeExcelData(
  ...
  if (syncType === 'first_sync') {
    ...
    // Combinar todas las fechas únicas: Excel + todas las yukyuDates locales existentes
    // (localDates ya incluye las aprobaciones locales previas)
    const allDates = new Set([...excelDates, ...localDates]);
