...
export function canApproveLeave(
  ...
  if (record.type === 'paid') {
    // 4. Verificar balance suficiente (半日=0.5, 全日=1.0)
    const balance = getEmployeeBalance(employee);
    const requiredDays = record.duration === 'half' ? 0.5 : 1;
    ...
    // 5. Verificar fecha no duplicada (检查 full y :half variants)
    const yukyuDates = employee.yukyuDates || [];
    const existsAsFull = yukyuDates.includes(record.date);
    const existsAsHalf = yukyuDates.includes(`${record.date}:half`);
...
export function isDateAlreadyTaken(employee: Employee, date: string): boolean {
  const yukyuDates = employee.yukyuDates || [];
  return yukyuDates.includes(date) || yukyuDates.includes(`${date}:half`);
}
